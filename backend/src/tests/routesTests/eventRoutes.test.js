...
jest.mock('../../middleware/eventMiddleware', () => ({
  validateEventInput: jest.fn((req, res, next) => next())
}));
...
describe('Event Routes', () => {
  ...
  describe('POST /', () => {
    it('should route to createEvent and use all middlewares', async () => {
      const response = await request(app)
        .post('/api/events')
    ...
    it('should handle validation errors', async () => {
      ...
      validateEventInput.mockImplementationOnce((req, res, next) => {
      ...
      expect(verifyToken).toHaveBeenCalled();
      expect(verifyAdmin).toHaveBeenCalled();
      expect(eventController.createEvent).not.toHaveBeenCalled();
    });
  });

  describe('PUT /:id', () => {
    it('should route to updateEvent and use all middlewares', async () => {
      const response = await request(app)
        .put('/api/events/1')
  ...
  describe('DELETE /:id', () => {
    it('should route to deleteEvent and use all middlewares', async () => {
      const response = await request(app)
        .delete('/api/events/1')
        .expect(200);
